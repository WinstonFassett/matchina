...
test.describe("HSM Traffic Light Flattened Mermaid Flowchart", () => {
  test("verify flowchart structure and styling", async ({ page }) => {
    ...
    page.on("console", (msg) => {
      if (
        msg.text().includes("graph LR") ||
        msg.text().includes("Generated") ||
        msg.text().includes("FLOWCHART DEBUG")
      ) {
    ...
    for (let i = 0; i < rectCount; i++) {
      ...
      const fill = await rect.evaluate(
        (el) => window.getComputedStyle(el).fill
      );
      const stroke = await rect.evaluate(
    ...
    for (let i = 0; i < labelCount; i++) {
      ...
      for (let j = 0; j < labelRectCount; j++) {
        ...
        const labelFill = await labelRect.evaluate(
          (el) => window.getComputedStyle(el).fill
        );
        const labelStroke = await labelRect.evaluate(
          (el) => window.getComputedStyle(el).stroke
        );
        console.log(
          `  Label rect ${j} fill: ${labelFill}, stroke: ${labelStroke}`
        );
      }

      const labelText = label.locator("span, text, .nodeLabel");
      if ((await labelText.count()) > 0) {
        const textColor = await labelText
          .first()
          .evaluate(
            (el) =>
        ...
        const textBg = await labelText
          .first()
          .evaluate((el) => window.getComputedStyle(el).backgroundColor);
        console.log(
          `Cluster label ${i} text color: ${textColor}, bg: ${textBg}`
        );
      }
    }
