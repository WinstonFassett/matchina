...
test.describe("Edge Interactivity Verification", () => {
  test("edge labels should have interactive classes", async ({ page }) => {
    ...
    const interactiveEdges = await page
      .locator(".edgeLabel p.edge-interactive")
      .count();
    const activeEdges = await page.locator(".edgeLabel p.edge-active").count();
    const inactiveEdges = await page
      .locator(".edgeLabel p.edge-inactive")
      .count();

    console.log(
      `Interactive: ${interactiveEdges}, Active: ${activeEdges}, Inactive: ${inactiveEdges}`
    );
  ...
  test("clicking active edge should trigger state transition", async ({
    page,
  }) => {
    await page.goto(BASE_URL);
    await page.waitForLoadState("networkidle");
    ...
    if ((await activeEdge.count()) > 0) {
      ...
      const newActiveEdges = await page
        .locator(".edgeLabel p.edge-active")
        .allTextContents();
