...
test.describe("Self-loop arrow positioning", () => {
  ...
  test("self-loop arrows should connect properly to node terminals", async ({
    page,
  }) => {
    ...
    for (let i = 0; i < selfLoopEdges.length; i++) {
      ...
      const distanceFromExpected = Math.sqrt(
        Math.pow(startX - expectedTerminal.x, 2) +
          Math.pow(startY - expectedTerminal.y, 2)
      );
      ...
      console.log(
        `Self-loop ${i}: Start point (${startX}, ${startY}), Expected terminal (${expectedTerminal.x}, ${expectedTerminal.y}), Distance: ${distanceFromExpected}px`
      );
    }
  ...
  test("self-loop arrows should have proper circular curves", async ({
    page,
  }) => {
  ...
  test("self-loop labels should be positioned outside the loops", async ({
    ...
    for (let i = 0; i < (await labelDivs.count()); i++) {
      ...
      const distance = Math.sqrt(
        Math.pow(labelCenterX - nodeCenterX, 2) +
          Math.pow(labelCenterY - nodeCenterY, 2)
      );
