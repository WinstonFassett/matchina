    ...
    modes.forEach((mode) => {
      visualizers.forEach((visualizer) => {
        test(`${theme} - ${mode} - ${visualizer}`, async ({ page }) => {
          ...
          ).toHaveScreenshot(`${visualizer}-${theme}-${mode}-1-inactive.png`);
          ...
          await expect(
            page.locator(SELECTORS.fullInteractiveArea).first()
          ).toHaveScreenshot(`${visualizer}-${theme}-${mode}-2-typing.png`);
          ...
          await expect(
            page.locator(SELECTORS.fullInteractiveArea).first()
          ).toHaveScreenshot(`${visualizer}-${theme}-${mode}-3-selected.png`);
        });
      });
    });
  });
