...
export function createVisualMatrixTests(exampleName: keyof typeof EXAMPLES) {
  ...
  const combinations = generateTestMatrix(themes, modes, [
    ...VISUALIZERS[config.preset],
  ]);
    ...
    combinations.forEach((combination) => {
      ...
      test(`${theme} - ${mode} - ${visualizer}`, async ({ page }) => {
        ...
        await takeScreenshot(
          page,
          `${exampleName}-${theme}-${mode}-${visualizer}-1-initial`
        );
        ...
        if (exampleName === "hsm-combobox") {
          ...
          await takeScreenshot(
            page,
            `${exampleName}-${theme}-${mode}-${visualizer}-2-typing`
          );
          ...
          await takeScreenshot(
            page,
            `${exampleName}-${theme}-${mode}-${visualizer}-3-selected`
          );
        }
      });
    });
