...
## 🔴 P0: CRITICAL - Visualizer Picker Chaos
...
**✅ Using `MachineVisualizer` (correct, unified):**
- `toggle/example.tsx`

**⚠️ Using `MachineExampleWithChart` (old, has broken "Basic"):**
...
**❌ Using `VisualizerDemo` (duplicate Force Graph, breaks examples):**
...
const visualizers = useMemo(() => [
  ...
  { key: 'forcegraph', label: 'Force Graph', ... }  // ← DUPLICATE!
], []);
```
...
- **Issue:** Gray background → rich purple with black text (hard to read)
...
| Standardize keys: `force-graph` → `forcegraph` | 2 files | No |
| Standardize labels: `Force Graph` → `ForceGraph`, `State Chart` → `Statechart` | 3 files | No |
| Migrate 17 examples from `MachineExampleWithChart` → `MachineVisualizer` | 17 files | No |
| Migrate 6 examples from `VisualizerDemo` → `MachineVisualizer` | 6 files | No |
...
1. ✅ Build passes
2. ✅ Preview deploys
3. ✅ ~~Remove duplicate Force Graph (WS1)~~ → Full migration completed
4. ✅ ~~Remove "Basic" option (WS1)~~ → Full migration completed
...
### 🟠 P1 BUG: ReactFlow Height Missing
