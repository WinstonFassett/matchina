import { createMachine, defineStates, facade } from "@lib/src";
...
export function createRPSMachine() {
  const machine = facade(
    ...
    // () => gameStates.WaitingForPlayer(0, 0) // Initial state with scores
    
  );
  
  // Extend with game-specific logic
  const extendedMachine = Object.assign(machine, {
    selectMove: (move: Move) => {
      // Player selects a move
      const { playerScore, computerScore } = machine.state.data;
      machine.selectMove(move, playerScore, computerScore);
    },
    
    computerSelectMove: () => {
      // Get current state data
      const { playerMove, playerScore, computerScore } = machine.state.data as any;
      ...
      if (newPlayerScore >= 3) {
        machine.computerSelectMove("player", newPlayerScore, newComputerScore);
        return;
      }
      
      if (newComputerScore >= 3) {
        machine.computerSelectMove("computer", newPlayerScore, newComputerScore);
        return;
      }
      
      // Game continues, show round result
      machine.computerSelectMove(
        playerMove, 
        computerMove, 
        roundWinner, 
        newPlayerScore, 
    ...
    nextRound: () => {
      const { playerScore, computerScore } = machine.state.data;
      machine.nextRound(playerScore, computerScore);
    },
