...
export function useStateEffects<S extends StateMatchbox<string, any>>(
  state: S,
  getEffects = (state: StateMatchbox<string, any>): Effect<S>[] | undefined =>
    (state.data as any).effects,
) {
  useEffect(() => {
...
function resolveState<S extends AnyStatesFactory>(
...
type TransitionEntry<T extends FactoryMachineTransitions<any>> = T extends {
