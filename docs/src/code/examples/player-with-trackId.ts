...
export const player = matchina(
  ...
  {
    Idle: { start: (trackId: string) => player => player.machine.states.Playing(trackId) },
    Playing: {
      pause: () => player => player.machine.states.Paused(player.from.state.data.trackId),
      stop: () => player => player.machine.states.Stopped(),
    },
    Paused: {
      resume: () => player => player.machine.states.Playing(player.from.state.data.trackId),
      stop: () => player => player.machine.states.Stopped(),
    },
    Stopped: { start: (trackId: string) => player => player.machine.states.Playing(trackId) },
  },
  "Idle"
);
...
const isPlayingTrack123 = player.state.is("Playing") && player.state.data.trackId === "track-123"; // true
