...
function adjustBrightness(light: ReturnType<typeof Light.Off> | ReturnType<typeof Light.On>, adjustment: number) {
  try {
    // Try to cast to "On" state
    const onLight = light.as("On");
    
    // If successful, create a new light with adjusted brightness
    const newBrightness = Math.max(0, Math.min(100, onLight.data.percentage + adjustment));
