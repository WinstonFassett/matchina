...
export function SimpleFetchDemo() {
  ...
  const def = useMemo(
    () => getXStateDefinition(fetcher.machine),
    [fetcher.machine],
  );
  const actions = useMemo(
    () => createApi(fetcher.machine, fetcher.state.key as any),
    [fetcher.state.key],
  );
  return (
    <div>
      ...
      {fetcher.state.match(
        {
          Idle: () => (
            <button onClick={() => actions.execute(1000)}>Say hello</button>
          ),
