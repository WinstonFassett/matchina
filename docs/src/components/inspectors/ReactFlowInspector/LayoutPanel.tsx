      ...
      {isOpen && (
        <div className="border-t p-4 space-y-4 max-h-96 overflow-y-auto">
          ...
          {algorithmConfig.hasLayerSpacing && (
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">
                {options.algorithm === 'force' ? 'Repulsion Strength' : 
                 algorithmConfig.supportsDirection ? 'Layer Spacing' : 'Distance'}: {options.layerSpacing}px
              </label>
              <input
                type="range"
          ...
          {algorithmConfig.hasEdgeNodeSpacing && (
            <div>
              ...
              <input
                ...
                onChange={(e) => handleChange('edgeNodeSpacing', parseInt(e.target.value))}
                className="w-full accent-blue-600"
              />
            </div>
          )}
          ...
          {algorithmConfig.hasThoroughness && (
            <div>
              ...
              <input
                ...
                onChange={(e) => handleChange('thoroughness', parseInt(e.target.value))}
                className="w-full accent-blue-600"
              />
              <p className="text-xs text-gray-500 mt-1">Higher values = better quality, slower layout</p>
            </div>
          )}
