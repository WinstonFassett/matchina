...
## Architecture / Dependencies
 ...
 <Mermaid client:only="react"
  ...
  subgraph RouterLib[Router Library]
    ...
    Defs[defineRouteBoxes]
    Matcher[matcher standalone]
  ...
  History <--> Store
  History -->|resolve path| Matcher
  Matcher -->|match result| History
...
- **History adapter**: Builds URLs (base/hash), listens to `popstate`, resolves routes via `matchAll()`, and dispatches results to the store.
- **Route boxes**: Generated by `defineRouteBoxes`, provide `to()` (URL building) and `match/matchAll()` for resolution.
