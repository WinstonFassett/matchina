...
A hierarchical Search Bar machine with an `Inactive → Active` parent flow. The `Active` state owns a nested child machine that models `Empty`, `TextEntry`, `Results`, and `Error`. Input events are routed child-first to keep the parent small and focused.
...
- Uses `submachine` to embed a child machine into the `Active` state.
