...
This example demonstrates a counter state machine that tracks a numeric value with increment, decrement, and reset operations. It shows how to add and manage data within state machines.

## State Definition

For our counter, we'll define states that represent different value ranges:
...
Each state carries a `value` property, but we separate them into different states to represent the semantic meaning of the counter's current range.

## Transitions

Next, we define transitions that modify the counter's value. We'll define them inline when creating the machine to maintain full TypeScript inference:

## Creating the Machine

Now we create the counter machine with both the states and transitions:
...
const counterMachine = matchina(
  counterStates,
  {
    Zero: {
      ...
      // No need to define reset since we're already at Zero
    },
    Positive: {
      increment: (state) => ({
        ...state,
