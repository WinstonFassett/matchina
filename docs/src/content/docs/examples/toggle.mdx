...
Next, we define the transitions between these states:
...
We've defined two transitions for each state:
- In the `On` state, either `toggle` or `turnOff` will transition to the `Off` state
- In the `Off` state, either `toggle` or `turnOn` will transition to the `On` state
