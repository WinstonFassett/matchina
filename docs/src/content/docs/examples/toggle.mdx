...
import ToggleExample from "@code/examples/toggle";
...
- Built-in pattern matching for conditional rendering

The implementation uses the `facade` function which combines state and transition definitions into a simple state machine.
...
- The toggle example is often used as the "Hello World" of state machines
- The visual component uses conditional styling based on the current state
- You can trigger transitions using either the generic `toggle` method or the specific `turnOn`/`turnOff` methods
