...
- **Effects map:** You can batch assign effect handlers using `on: { effects: { tick: fn, jump: fn } }`.
  ...
  onLifecycle(machine, {
    "*": {
      on: {
        ...
        effects: {
          jump: (ev) => console.log("jump effect", ev),
        },
      }
    }
  });
...
onLifecycle(machine, {
  Idle: {
    on: {
      ...
      effects: {
        tick: (ev) => {
          console.log("Idle.tick effect:", ev.type, ev.from.key, ev.to.key);
        },
      },
