...
if (state.is("Playing")) {
  ...
  console.log(`Playing ${trackId} since ${new Date(startTime).toLocaleTimeString()}`);
} 
...
const ui = state.match({
  ...
  Playing: ({ trackId, startTime }) => `Playing ${trackId} (started at ${new Date(startTime).toLocaleTimeString()})`,
...
const Shape = matchbox({
  ...
  Rectangle: (width: number, height: number, color: string = "black") => ({ width, height, color }),
  Triangle: (base: number, height: number, color: string = "black") => ({ base, height, color }),
});
