...
<Code code={lightTypeGuardsRaw} lang="ts" />

### Cast using `as()`

Use the `as` method for type-safe casting with runtime validation:

<Code code={lightCastingRaw} lang="ts" />
