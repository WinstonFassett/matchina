...
<Code code={matchSetupRaw} lang="ts" />
...
<Code code={lightTypeGuardsRaw} lang="ts" meta="twoslash" />

### Cast using `as()`

Use the `as` method for type-safe casting with runtime validation:

<Code code={lightCastingRaw} lang="ts" meta="twoslash" />
