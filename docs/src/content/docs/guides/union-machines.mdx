...
console.log(getArea(triangle));    // 60
...
try {
  ...
  console.log(asCircle.data.radius);  // Works fine
  
  // Runtime error - rectangle is not a Circle
...
```ts
import { matchbox, createFactoryMachine } from "matchina";
...
const authMachine = createFactoryMachine(
  AuthState,
  {
    LoggedOut: {
