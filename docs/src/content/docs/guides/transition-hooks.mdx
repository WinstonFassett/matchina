...
The `transitionHooks` function provides a flat, composable API for registering lifecycle hooks on a [FactoryMachine](../reference/factory-machine). Each hook config can specify matching criteria (`from`, `to`, `type`) and any lifecycle phase (`effect`, `before`, `after`, `guard`, etc.).
...
## See Also
- [API Reference: transitionHooks](../reference/functions/transitionhooks)
- [Stopwatch Example Using Transition Hooks](../examples/stopwatch-using-transition-hooks-instead-of-useeffect)
