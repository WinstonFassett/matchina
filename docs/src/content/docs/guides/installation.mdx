...
[ArrowJS](https://arrow-js.com/) is one good fit (no-build, tagged template literals, small). [Live demo →](/matchina/cdn-demos/standalone-arrow/)

```html
<script type="module">
  const { matchina, defineStates } = await import(
    'https://esm.sh/matchina?bundle&exports=matchina,defineStates'
  );
  const { reactive, html } = await import('https://esm.sh/@arrow-js/core');
