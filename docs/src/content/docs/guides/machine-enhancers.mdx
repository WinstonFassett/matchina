...
<TabItem label="addEventApi">
  Use `addEventApi` when: - You want to enhance an existing machine with a flat
...
<TabItem label="zen">
  Use `zen` when: - You need both event methods and state access in a clean
...
const toggleApi = eventApi(toggleMachine);
...
const api = eventApi(counterMachine);
...
You can also use the `addEventApi` helper to add the API directly to your machine:
...
const enhancedMachine = addEventApi(toggleMachine);
...
The `zen` enhancer builds on top of `eventApi` and provides a clean, minimal interface for working with state machines. It combines the event API with easy access to the machine's state and change, along with a setup method for integrations.
{/*
```ts twoslash
// @noErrors
...
You can combine wrappers for different scenarios. Here's an example of using `eventApi` internally and exposing a `pure` interface externally:

```ts twoslash
// @noErrors
import { createMachine } from "matchina";
import { eventApi, pure } from "matchina";
...
const api = eventApi(authMachine);
...
- Internal code uses the `eventApi` wrapper for typed event methods
