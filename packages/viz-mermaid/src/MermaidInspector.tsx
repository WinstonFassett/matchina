...
export function generateFlowchart(shape: MachineShape): string {
  ...
  for (const [stateKey, transitions] of shape.transitions.entries()) {
    ...
    for (const [event, target] of transitions.entries()) {
      const targetId = getStateId(target);
      rows.push(
        `    ${fromId}-->|${fromId}<br>${event}|${targetId}["${target}"]`
      );
    }
  }
...
export const MermaidInspector = React.memo(function MermaidInspector({
  ...
  console.log("MermaidInspector called with:", {
    shape,
    currentStateKey,
    diagramType,
  });
  ...
  const applyHighlights = () => {
    ...
    container
      .querySelectorAll(".mermaid-active-state, .state-highlight")
      .forEach((el) => {
        el.classList.remove("mermaid-active-state", "state-highlight");
      });
    container
      .querySelectorAll(".mermaid-active-edge, .edge-active")
      .forEach((el) => {
    ...
    for (const selector of stateSelectors) {
      ...
      if (stateNode) {
        console.log(
          "[MermaidHighlight] Found state with selector:",
          selector,
          stateNode
        );
        break;
    ...
    if (stateNode) {
      ...
      let shapeEl =
        stateNode.tagName === "rect" || stateNode.tagName === "path"
          ? stateNode
    ...
    if (stateTransitions) {
      ...
      const edgePaths = container.querySelectorAll(
        ".edgePath path, path.transition"
      );
      const edgeLabels = container.querySelectorAll(
        ".edgeLabel, .edgeTerminals"
      );
            ...
            if (parentG) {
              ...
              if (path) {
                ...
                (path as SVGPathElement).style.stroke =
                  "var(--sl-color-accent-high, #ef4444)";
...
export const MermaidInspectorWithSettings = React.memo(
  function MermaidInspectorWithSettings({
    config,
    stateKey,
    actions,
    interactive = true,
    ...
    console.log("MermaidInspectorWithSettings called with:", {
      config,
      stateKey,
      machine,
    });
    ...
    const shape = useMemo(() => {
      ...
      if (config) {
        ...
        try {
          ...
          console.error(
            "MermaidInspector: Failed to create shape from config:",
            error
          );
