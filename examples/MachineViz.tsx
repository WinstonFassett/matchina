...
import { FactoryMachine, FactoryMachineTransitions } from "../../src";
import Mermaid from "./Mermaid";
...
// classDef active fill:aquamarine;
  ...
  function addMachine (config: any) {
      ...
      if (stateTransitions) {
        Object.keys(stateTransitions).forEach((eventType) => {
          ...
          if (targetStateEntry) {
            ...
            rows.push(`    ${stateKey}-->|${eventType ? `${stateKey}<br>${eventType}` : `${stateKey}<br>AUTO`}|${targetStateKey}[${targetStateKey}]`);
    ...
    //   rows.push(`${indent()}}`)
    // })
    const unrenderedStates = Object.keys(config).filter(state => !renderedStates.has(state))
...
export const StateMachineMermaidDiagram = memo(({ config, stateKey, actions }: { config: any, stateKey: string, actions?: Record<string, () => void> }) => {
  const [id] = useState((lastId++).toString())
  const { states } = config
  const chart = useMemo(
    () => toStateDiagram(config, id), 
    [states]
  );
