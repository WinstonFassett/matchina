...
export type SubmachineMarker = {
  ...
  statesConfig: any; // TaggedTypes used by defineStates
...
export function defineSubmachine(
  states: Record<string, any> | StateMatchboxFactory<any>,
  transitions: Record<string, any>,
  initial: string
): SubmachineMarker {
...
export function flattenMachineDefinition<SF extends StateMatchboxFactory<any>, T extends FactoryMachineTransitions<SF>, I extends keyof SF>(
  ...
  // Build a states factory from flat keys (simple undefined payloads for now)
  const flatStatesFactory = defineStates(
    Object.fromEntries(Object.keys(flattened.states).map((k) => [k, undefined])) as any
  );
