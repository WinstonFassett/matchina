...
describe('applyMiddleware', () => {
  ...
  it('applies a single middleware function', () => {
    const originalFn = vi.fn((a: number, b: number) => a + b);
    const middleware = vi.fn((next) => (a: number, b: number) => next(a + 1, b + 1));
  ...
  it('applies multiple middleware functions in order', () => {
    const originalFn = vi.fn((a: number, b: number) => a + b);
    const middleware1 = vi.fn((next) => (a: number, b: number) => next(a + 1, b + 1));
    const middleware2 = vi.fn((next) => (a: number, b: number) => next(a * 2, b * 2));
