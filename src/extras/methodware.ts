...
function composeEnhancers<S,
  ...
  let finalEnhancer: MethodEnhancer<S, K> = ((orig: F, ...args: Parameters<F>) => {
    console.log('ORIG')
    return orig(...args)}) as MethodEnhancer<S, K>
  let i = enhancers.length
  const copy = enhancers.slice().reverse()
  for (const enhancer of copy) {
    ...
    finalEnhancer = ((innerMethod: Method<S, K>, ...args: Parameters<F>) => {
      ...
      return enhancer(
        ((...args: Parameters<F>) => nextEnhancer(innerMethod, ...args)) as Method<S, K>,
        ...args
      ) as ReturnType<F>
