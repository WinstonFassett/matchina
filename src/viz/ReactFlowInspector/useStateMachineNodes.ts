...
export const useStateMachineNodes = (
  ...
  const states = useMemo(() => {
    ...
    const extractStates = (shapeObj: any, prefix = '') => {
      ...
      const entries = shapeObj.states instanceof Map
  ...
  const updateNodeStates = useCallback(() => {
    setNodes((currentNodes) =>
      currentNodes.map((node) => {
        // Handle both flat and hierarchical state names
        const nodeState = node.id;
        let isActive = false;
        let isPrevious = false;

        if (nodeState.includes('.')) {
          // Hierarchical - check if current state ends with this state
          isActive = currentState === nodeState || currentState.endsWith('.' + nodeState.split('.').pop());
          if (previousState) {
            isPrevious = previousState === nodeState || previousState.endsWith('.' + nodeState.split('.').pop());
          }
        } else {
          // Flat - exact match
          isActive = currentState === nodeState;
          if (previousState) {
            isPrevious = previousState === nodeState;
          }
        }
