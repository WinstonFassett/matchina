...
export type StatesOf<M> = M extends { states: infer S }
  ? keyof S & string
  : never;

export type EventsOf<M, K extends StatesOf<M>> = M extends {
  transitions: infer T;
}
  ? K extends keyof T
    ? keyof T[K] & string
...
export type AllEventsOf<M> = M extends { transitions: infer T }
  ? keyof T[keyof T & string] & string
  : never;

export type ChildOf<M, K extends StatesOf<M>> = M extends { states: infer S }
  ? K extends keyof S
...
export type ActiveEvents<M, K extends StatesOf<M>> = [ChildOf<M, K>] extends [
  never,
]
  ? EventsOf<M, K>
...
export function sendWhen<
  M extends { send: (...a: any[]) => any },
  K extends StatesOf<M>,
  E extends ActiveEvents<M, K>,
>(machine: M, _key: K, type: E, ...params: any[]) {
