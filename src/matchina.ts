...
import { StateFromFactory, TransitionConfig, createFactoryMachine } from "./factory-machine";
...
>(
  ...
  init: StateFromFactory<States<S>> | ((states: States<S>, transitions: T) => StateFromFactory<States<S>>)
) {
  const states = defineStates(stateConfig) as States<S>
  const transitions = typeof transitionConfig === "function" ? transitionConfig(states) : transitionConfig
  const initialState = typeof init === "function" ? init(states, transitions) : init
  ...
  return {
    ...
    get change() { return machine.getChange() },
