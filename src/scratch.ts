...
// Implementation
type TargetStates = { [K in keyof MyTransitions]: ReturnType<MyTransitions[K]> }
type TargetStatesForParams1<P extends any[]> = { 
  [K in keyof MyTransitions]: MyTransitions[K] extends (...args: P) => any ? ReturnType<MyTransitions[K]> : never 
}

// Test
let x = {} as TargetStates; // should be { Idle: MyStates['Loading'], Loading: MyStates['Success'] }
...
type TargetStatesForParams<P extends any[]> = {
  [K in keyof MyTransitions]: 
    (ParamType<MyTransitions[K]> extends [P[0], ...any[]] ? ReturnType<MyTransitions[K]> : never)
}
