import { notify } from "../../machine-hooks";
...
export function runRegistrants<V, K extends string, T>(
  target: T,
  kind: K,
  type: any,
  params: any[],
...
export function getRegistrants<V, K extends string, T>(
  target: T,
  kind: K,
  type: any,
) {
...
>(target: any, init: (records: Record<K, F>) => void, kind = LISTENERS as K) {
  ...
  return {
    register: <Type extends string>(type: Type, fn: F) =>
      register(target, fn, type, kind),
    getRegistrants: <Type extends string>(type: Type) =>
      getRegistrants<F, K, T>(target, kind, type),
  };
}
 ...
 */
const useProperty =
  <T, K extends string, R>(key: K, fn: () => [T, Disposer]) =>
  (target: R) => {
...
export function register<F extends (...params: any[]) => any>(
  target: any,
  fn: F,
  type = DEFAULT,
  kind = LISTENERS,
