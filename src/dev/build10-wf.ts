...
interface CanDefineStates<C> {
  defineStates<T extends States>(states: T): BuilderForContext<C & { states: T }>;
}

interface CanSetInitialState<C> {
  setInitialState<T extends string>(initialState: T): BuilderForContext<C &{ initialState: T }>;
}
interface CanDefineTransitions<C> {
  defineTransitions<T extends Transitions>(transitions: T): BuilderForContext<C & { transitions: T }>;
}
...
type CanCreateStateMachine<C> = {
  ...
  (context:Partial<StateMachineContext> &  RemainingProperties<StateMachineContext, C>) => StateMachine
}

type RemainingProperties<Required, Present, Match = any> =
  Pick<Required, Exclude<keyof Required, keyof Present> & (Match extends any ? {[K in keyof Required]: Required[K] extends Match ? K : never}[keyof Required] : never)>;
...
const initialStateBeforeTransitions = builderWithStates.setInitialState('Idle' as const);
...
const initialStateWithTransitions = initialStateBeforeTransitions.defineTransitions({
  Idle: { start: 'Working' },
  Working: { finish: 'Done' },
} as const);
...
const initialStateAfterTRansitions = transitionsBeforeInitialState.setInitialState('Idle' as const)
