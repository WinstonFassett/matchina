...
export function methodV2<K extends string, T extends HasMethod<K> = HasMethod<K>>(methodName: K, target: T, extend: (inner: T[K]) => T[K]) {
  const original = target[methodName];  
...
export const methodUseV2 =
  <K extends string>(methodName: K) =>
  <T extends HasMethod<K>>(fn: (inner: T[K]) => T[K]) => (target: T) =>
    methodV2(methodName, target, fn(target[methodName])); 

export const methodListen = <K extends string>(methodName: K) => <T extends HasMethod<K>>(fn: T[K]) => (target: T) => {
