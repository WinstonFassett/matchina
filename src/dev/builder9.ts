...
export interface CanDefineStates {
  defineStates(states: States): MachineBuilder<{ states: States } & Omit<BaseContext, 'states'>>;
}

export interface CanDefineTransitions {
  defineTransitions(transitions: Transitions): MachineBuilder<{ transitions: Transitions } & Omit<BaseContext, 'transitions'>>;
}

export interface CanSetInitialState {
  setInitialState(initialState: string): MachineBuilder<{ initialState: string } & Omit<BaseContext, 'initialState'>>;
}
...
export type MachineBuilder<C extends BaseContext> = 
  (C extends { states: undefined } ? CanDefineStates : {}) &
