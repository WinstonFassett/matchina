...
export type TUnionToIntersection<T> = (T extends any ? (x: T) => any : never) extends (x: infer R) => any ? R : never;
...
export type RemainingProperties<Required, Present, Match = any> = Pick<
  Required, Exclude<keyof Required, keyof Present> &
  (Match extends any ? {
    [K in keyof Required]: Required[K] extends Match ? K : never;
  }[keyof Required] : never)
>;
